...
func CalculateMonthlySavings(plan MonthlyExpensesPlan) float64 {
	requiredFunds, err := CalculateRetirementFunds(plan)
	if err != nil {
		return 0
	}
	monthlySavings := (requiredFunds + plan.AllAsset + plan.NursingHousePrice) / float64(plan.YearsUntilRetirement*12)
